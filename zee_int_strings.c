...
int get_base_len(unsigned int num, unsigned int base);
void fill_base_buff(unsigned int num, unsigned int base,
...
char *get_int(int num)
{
	...
	fill_base_buff(temp, 10, ret, length);
	if (num < 0 || num_l < 0)
		ret[0] = '-';
...
/**
 * get_abs - gets the absolute value of an integer
...
/**
 * get_base_len - gets length of buffer needed for an unsigned int
...
/**
 * fill_base_buff - fills buffer with correct numbers up to base 36
